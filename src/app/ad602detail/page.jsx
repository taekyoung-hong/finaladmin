...
const DetailPage = () => {
    // URL에서 box_idx를 가져옵니다.
    const searchParams = useSearchParams(); // URL에서 box_idx를 추출
    const box_idx = searchParams.get("box_idx");  // router 객체를 가져옵니다.
    console.log("box_idx",box_idx);
    ...
    useEffect(() => {
        if (box_idx) {
            const fetchData = async () => {
                try {   
                    // API 호출하여 약국 정보 가져오기
                    const response = await axios.get(`http://localhost:8080/api/box_info/detail/${box_idx}`);
                    const { data } = response.data; // { data: { box_name, box_address, box_lat, box_long } }
                    ...
                    setBox({
                        ...
                        box_lat: parseFloat(data.box_lat) || 37.49676871972202,
                        box_long: parseFloat(data.box_long) || 127.02474726969814,
                    });

                    setState({
                        center: {
                            lat: parseFloat(data.box_lat) || 37.49676871972202,
                            lng: parseFloat(data.box_long) || 127.02474726969814,
                        },
                        isPanto: true,
                    });
                } catch (error) {
                    console.error("Error fetching boxmacy details:", error);
    ...
    }, [box_idx]); // box_idx가 바뀔 때마다 데이터 새로 가져오기
    ...
    const handleSave = async () => {
        try {
            ...
            // axios POST 요청
            axios.post("http://localhost:8080/api/box_info/update", data, {
                headers: {
                    ...
                    <div className={adcommons.adcommons__main_container_box}>
                        ...
                        <div className={adcommons.adcommons__box}>
                            <TextField
                                ...
                                id="box_name"
                                name="box_name"
                                value={box.box_name || ''}  // 값이 없으면 빈 문자열로 처리
                    ...
                    <div className={adcommons.adcommons__sub1_container_box}>
                        ...
                        <div className={adcommons.adcommons__box}>
                            <TextField
                                ...
                                id="box_address"
                                name="box_address"
                                value={box.box_address || ''}  // 값이 없으면 빈 문자열로 처리
                                onChange={handleChange}
                            />
                        </div>
                    </div>

...
const DetailPage = () => {
    ...
    const drug_side_effect_idx = searchParams.get("drug_side_effect_idx");  // router 객체를 가져옵니다.
    console.log("현재 drug_side_effect_idx:", drug_side_effect_idx);  // 콘솔에 출력하여 값 확인
    ...
    useEffect(() => {
        if (drug_side_effect_idx) {
            const fetchData = async () => {
                try {
                    // API 호출하여 약국 정보 가져오기
                    const response = await axios.get(`http://localhost:8080/api/drug_side_effect/detail/${drug_side_effect_idx}`);
    ...
    }, [drug_side_effect_idx]); // phar_idx가 바뀔 때마다 데이터 새로 가져오기
